...
async def show_profile(message: Message, pool):
    async with pool.acquire() as conn:  # взяли соединение
        ...
        calc = Calculate()
        user_days_without_smoke = calc.days_without_smoke(
            str(user_and_cigarette.get("start_date"))
        )
        total_not_smoked_cig = calc.total_not_smoked_cigarettes(
            int(user_and_cigarette.get("cigarettes_per_day"))
        )
        user_total_saved_money = calc.total_saved_money(
            user_cig_in_pack, user_cig_price
        )
