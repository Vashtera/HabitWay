...
async def add_user(tg_id: int, name: str, cigarette_id: int, date: Any, conn):
    return await conn.execute(
        "INSERT INTO users (tg_id, fullname, cigarette_id, start_date) VALUES ($1, $2, $3, $4)",
        tg_id,
        name,
        cigarette_id,
        date,
...
async def add_cigarettes(cig_in_pack: int, cig_per_day: int, cig_price: float, conn):
...
async def change_the_price(new_price: float, conn):
    return await conn.execute(
        "UPDATE cigarettes AS c SET cigarette_price = ($1) FROM users AS u WHERE c.cigarette_id = u.cigarette_id",
        new_price,
    )
